#!/usr/bin/env node
/**
 * Decide whether to patch-bump and npm publish signalk-control-panel-plugin.
 *
 * Publishes at most once per UTC day when plugin/, lib/ or public/ changed
...
const PACKAGE_NAME = 'signalk-control-panel-plugin'
const TRACKED_PATHS = ['plugin', 'lib', 'public']
...
function shouldPublish(opts) {
  ...
  if (files.length === 0) {
    return {
      publish: false,
      reason: 'no plugin, lib or public updates since last publish',
      files: []
    }
  }
  return {
    publish: true,
    reason: 'plugin, lib or public updated since last publish',
    files: files
  }
}
